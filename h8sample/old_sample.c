...
*/
    ...
    while(1)
    {
		...
		if ( dis == 1 ) {
			set_flg(FLGID_FLG1, dis_S_flg);
//			Led_setLight(D_LED_KIND_GREEN, D_LED_LIGHT_ON);    //デバッグ用
			SCI3_PutChar(3);
		
		} else if ( dis == 2) {
			set_flg(FLGID_FLG1, dis_S_flg);
//			Led_setLight(D_LED_KIND_ORANGE, D_LED_LIGHT_ON);    //デバッグ用
			SCI3_PutChar(4);
		
		} else if ( dis == 0)  {
//			Led_setLight(D_LED_KIND_ORANGE, D_LED_LIGHT_OFF);    //デバッグ用
//			Led_setLight(D_LED_KIND_GREEN, D_LED_LIGHT_OFF);    //デバッグ用
			SCI3_PutChar(5);
		}			
		dly_tsk(500);
